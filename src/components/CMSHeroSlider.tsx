...
const CMSHeroSlider = () => {
  ...
  return (
    ...
    >
      {/* Background Images dengan placeholder */}
      <div className="absolute inset-0">
        {visibleSlides.map((slide, index) => (
          ...
          >
            {/* Placeholder background dengan gradient */}
            <div className="w-full h-full bg-gradient-to-br from-blue-600 via-purple-600 to-teal-600">
              {/* Optional: Jika ada image src yang valid */}
              {slide.image.src && slide.image.src !== '/images/hero-car-1.jpg' && (
                <img
                  src={slide.image.src}
                  alt={slide.image.alt}
                  className={`w-full h-full object-cover object-${slide.image.position || 'center'}`}
                  onError={(e) => {
                    // Hide image jika error loading
      ...
      <div className="relative z-10 container mx-auto text-center px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {currentSlideData.badge?.isVisible && (
            <div className="mb-6 animate-bounce">
              <span 
                ...
                } ${currentSlideData.badge.textColor || 'text-white'} shadow-lg`}
              >
          ...
          {currentSlideData.subtitle && (
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-semibold text-gray-200 mb-6 animate-fade-in-delay">
              {currentSlideData.subtitle}
            </h2>
          )}
              ...
              <div className="space-y-3">
                {currentSlideData.features
                  ...
                  .map((feature, index) => (
                    <div 
                      key={feature.id} 
                      className="flex items-center justify-center text-green-400 animate-slide-in"
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center animate-fade-in-delay-4">
            ...
            {currentSlideData.ctaButton?.isVisible && (
              <Button
                ...
                } px-8 py-4 text-lg font-bold rounded-full transition-all duration-200 hover:scale-105 shadow-lg transform`}
                size="lg"
              >
                {currentSlideData.ctaButton.icon && (
                  <span className="mr-2 text-xl">{currentSlideData.ctaButton.icon}</span>
                )}
            ...
            {currentSlideData.secondaryCTA?.isVisible && (
              <div className="flex items-center">
                <Button
                  ...
                  } px-6 py-3 rounded-full font-bold hover:scale-105 transition-all duration-200 shadow-md`}
                  variant="secondary"
                >
      ...
      {/* Slide Counter */}
      <div className="absolute top-8 right-8 z-20 text-white/70 text-sm font-medium">
